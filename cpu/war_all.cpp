// Count the positions of n-card War (ranks 0..n-1, all distinct) whose game never ends: every order p of the
// cards and every split K = 1..n-1, player 1 (A) = p[0..K-1], player 2 (B) = the rest, n! (n-1) positions.
...
// usage: war_all n mode      build: g++ -O3 -march=native -fopenmp -o war_all war_all.cpp
#include <cstdio>
#include <cstdint>
#include <cstdlib>
#include <algorithm>
...
int main(int argc, char** argv) {
    ...
    for (int J = 0; J < (int)jobs.size(); J++) {
        ...
        do {
            for (int K = 1; K < n; K++) { S s; s.a = 0; s.b = 0; s.la = 0; s.lb = 0;
            for (int i = 0; i < n; i++) { if (i < K) { s.a |= (u64)p[i] << (4*s.la); s.la++; } else { s.b |= (u64)p[i] << (4*s.lb); s.lb++; } }
            long long r = run(s);
            total++;
            if (r < 0) cyc++; else if (r > longest) longest = r; }
        } while (std::next_permutation(p + 2, p + n));
    }
    printf("n=%d mode=%d positions=%lld cycling=%lld longest_finite=%lld\n", n, MODE, total, cyc, longest);
    return 0;
}
